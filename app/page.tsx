...
export default function Home() {
  ...
  const filteredDocuments = documents.filter((doc: any) => {
    ...
    const org = doc.organization || doc.agency || "";
    const type = doc.type || "";
    const desc = doc.description || "";

    const searchableText = `${title} ${org} ${type} ${desc}`.toLowerCase();
    const matchesQuery = searchableText.includes(query.trim().toLowerCase());

    const matchesOrganization =
      selectedOrganization === "すべて" ||
      org.includes(selectedOrganization);
            ...
            <div className="space-y-4">
              {sortedDocuments.map((doc: any, index: number) => (
                ...
                >
                  ...
                  <h2 className="mt-4 text-xl font-bold">{doc.title}</h2>
                  ...
                  <p className="mt-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-900">
                    ...
                    {doc.primarySourceReason || "公的機関が公式に公開した資料"}
                  </p>

                  <div className="mt-4 flex items-center justify-between gap-4 text-sm">
                    <span className="text-slate-500">{doc.publishedAt || doc.date}</span>
